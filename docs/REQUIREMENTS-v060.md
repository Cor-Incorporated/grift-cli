...
| REL-072 | v0.7.2 のリリース版数と定義版数を上げる。`__version__` = 0.7.2 / `DEFINITION_VERSION` = tep-v0.7.2-2026-09-07。面は 1 つも増えず、commit ゼロの actor で report が 1 つも出なくなっていた 0.7.0 回帰（`$.context_profile` が oneOf のどの形にも当たらない）を直す patch release である。`DEFINITION_VERSION` を上げる根拠は版数追随に加えて、commit ゼロの actor の `context_profile` の形が変わること（0.7.1 ではそもそも report が出なかった）。あわせて P0 反証スイートを ci.yml に入れ、publish.yml と同じ baseline pin であることを機械照合する | `version.py`; `action.yml`; CHANGELOG; `docs/migration-v072.md`; `.github/workflows/ci.yml`; `tests/test_v072_empty_actor.py`; `tests/test_publish_baseline_pin.py` | P0 反証スイート 43/43（0.7.1 は 42/43）; 片側変異 3 件（`prepare_inputs` の門番復元 / catalog の `language_composition` 固定復元 / ci.yml の pin 改変）がいずれも red | LOCALLY_VERIFIED（P0 43/43 rc=0、`test_v060_release_consistency.py` green、private registry `--verify-registry` rc=0 PASS、public registry PR tier verdict PASS かつ `--verify-registry --digest-only` rc=0、live gate summary `--verify-summary --expected-tier pr --expected-source-commit` VERIFIED、`scripts/v070_benchmark.py` self-contained 6/6 hold、`scripts/package_smoke.py` rc=0、intake parity VERIFIED 8/8。**golden baseline の migration は人間ゲート未通過**。dry-run の drift は D3×1 tool_version / D4×2 definition_version、D0-D2 = 0 で `baseline_update: forbidden_automatic`。`v060_golden.py` の `_HUMAN_APPROVER` が `human:` / `committee:` 以外の承認者を拒否するため、`scripts/v060_golden_baseline.py --case actor-partition --approve --requirement REL-072 --migration-id gold-actor-partition-0004 --approved-by human:<承認者> --approved-at <RFC3339>` を人間が実行するまで `test_v060_golden_runner.py::test_runner_executes_repo_and_verify_cli` は red） |
| REL-071 | v0.7.1 のリリース版数と定義版数を上げる。`__version__` = 0.7.1 / `DEFINITION_VERSION` = tep-v0.7.1-2026-09-06。面は 1 つも増えず、公開 Forge 取得の 2 つの欠陥（bot / app アカウントで exit 2、大きな commits ページの読み残し）を直す patch release である。golden baseline の drift は版数文字列 3 行のみで、観測値は 1 つも変わらない | `version.py`; CHANGELOG; `docs/migration-v071.md` | golden runner の drift 分類（D3×1 tool_version / D4×2 definition_version、D0-D2 = 0） | LOCALLY_VERIFIED（`test_v060_release_consistency.py` green、PR tier live gate verdict PASS、`scripts/v070_benchmark.py` self-contained 6/6 hold、intake parity VERIFIED 8/8。**golden baseline の migration は人間ゲート未通過**。`v060_golden.py` の `_HUMAN_APPROVER` が `human:` / `committee:` 以外の承認者を拒否するため、`scripts/v060_golden_baseline.py --case actor-partition --approve --requirement REL-071 --migration-id gold-actor-partition-0003 --approved-by human:<承認者> --approved-at <RFC3339>` を人間が実行するまで `test_v060_golden_runner.py::test_runner_executes_repo_and_verify_cli` は red） |
