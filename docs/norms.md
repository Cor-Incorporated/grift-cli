...
1. **目的拘束**: 明示的提出（`grift contribute`）で受け取ったデータは「TEP Report 集計と参照分布 vNext」の目的にのみ使用し、**それ以外（チャネル4: Grift SaaS・組織契約・営業・見積りへの転用を含む）には一切使わない**
2. **保持期間**: 次回年次 Report の発行まで
3. **撤回手続き**: (a) 提出者による自 payload 削除の PR (b) 受け口リポ（tep-contributions）の連絡先への依頼。非公開ドア経由の提出者身元は撤回時も開示されない
4. **推測リスクの開示**: payload に repo 名は含まれない（付記は opt-in）が、特徴の組合せから元 repo が推測されるリスクはゼロではない — この限界は提出確認文に毎回表示される
